...
- 本プログラムのUCは混合整数線形計画法（Mixed-integer linear programming: MILP）で定式化されている。詳細な定式化内容はREADMEの第４章[「最適化問題の定式化」](../README.md#目次)で確認することができる
  - 最適化の目的関数は最適化対象期間における全地域の総社会調達コスト最小化である。
  ...
  - 主な制約条件は以下の通りである。
    ...
    - 複数地域の系統運用制約
      ...
      - 調整力制約
      - 慣性定数制約
- 前回の最適化で決定された変数の一部を最適化対象の前時間帯に引き継ぐことで、連続した最適化（Rolling Optimization）を実現している。
  ![Rolling optimization image](./img/01/rolling-opt.png)

### 三次調整力の考慮方法

再生可能エネルギーの出力予測誤差に対応するための三次調整力の考慮方法は、フラグ[`formulation_type`](./06_config/03_unit_commitment.md#formulation_type)によって以下の２つから選択できる。

- **ΔkW市場なしタイプ `delta-kW-no-market`（既定）**：三次調整力を予備力量として必要量だけ確保する。目的関数に調整力調達費は計上しない。本フラグ導入前の既定の挙動と完全に同一である（後方互換）。
- **ΔkW入札タイプ `delta-kW-bid`**：需給調整市場での三次調整力の取引を考慮し、その価値を、前日計画では ΔkW（kW価値）、当日計画では kWh（電力量価値）として目的関数・制約に反映する。

ΔkW入札タイプを用いると、ΔkW市場なしタイプでは扱えなかった **三次調整力の調達費を含む経済性評価** が可能になり、需給調整市場の制度に整合した、より現実的な UC 計画を検討できる。両者の違いを下図に示す。

![Delta kw image](./img/01/delta_kw.png)

詳細は次を参照：[設定値（`formulation_type`）](06_config/03_unit_commitment.md#formulation_type) ／ [目的関数の定式化](04_formulation/01_objective_function.md) ／ [前日計画から当日計画への引き継ぎ](04_formulation/02_constraint/06_inheritance.md)
