...
- 列名が「name」の列については、ASCII文字（英数文字と記号）のみ使用可能です。日本語等の非ASCII文字が含まれている場合、CSVファイル読み込み時にバリデーションエラーが発生します。
- バリデーションエラーの際には、対象のCSVファイル名・列名・該当する値（当該ファイルの行番号付き）が表示されます。
