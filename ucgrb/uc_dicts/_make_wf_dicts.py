...
def _make_wf_dicts(uc_data, uc_dicts):
    ...
    try:
        ...
        uc_dicts.wf, uc_dicts.wf_para["R_GF_LFC_UP"] = result
        del _df
    except Exception as e:
        print("=" * 80)
        print("🚨 WF辞書作成エラー（WF_R_GF_LFC_UP.csv） 🚨")
        print("=" * 80)
        print("📁 問題のあるCSVファイル: WF_R_GF_LFC_UP.csv")
        print("🔧 処理中の関数: _make_wf_dicts")
        print("🔧 処理中の項目: 短周期変動予測誤差率（UP）")
        print(f"❌ エラーの種類: {type(e).__name__}")
        print(f"💬 エラーの詳細: {str(e)}")
        print("=" * 80)
        print("🔧 対処方法:")
        print("   1. WF_R_GF_LFC_UP.csvファイルの形式を確認してください")
    ...
    try:
        ...
        for i in uc_dicts.area:
            ...
            if "_df" not in locals():
                ...
                _df = pd.concat([_df, _ser[i]])
        result = gp.multidict(_df)
        uc_dicts.wf, uc_dicts.wf_para["R_GF_LFC_DOWN"] = result
        del _df
    except Exception as e:
        print("=" * 80)
        print("🚨 WF辞書作成エラー（WF_R_GF_LFC_DOWN.csv） 🚨")
        print("=" * 80)
        print("📁 問題のあるCSVファイル: WF_R_GF_LFC_DOWN.csv")
        print("🔧 処理中の関数: _make_wf_dicts")
        ...
        print("=" * 80)
        print("🔧 対処方法:")
        print("   1. WF_R_GF_LFC_DOWN.csvファイルの形式を確認してください")
        print("   2. 必須列（エリア名）が存在するか確認してください")
        print("   3. データ型が正しいか確認してください")
        print("   4. 時系列データの整合性を確認してください")
        print("=" * 80)
