...
# -*- coding: utf-8 -*-
"""
Tertiary (Down) グラフ生成モジュール.

各地域の三次調整力（下げ）を示すグラフを作成する。
"""
from .._append_col import _append_col
from .._make_constraint_chart import _make_constraint_chart


def make_tertiary_down_graph(ws, period_name, timeline, time_format, area, m, uc_data, uc_dicts):
    """
    各地域の三次調整力（下げ）グラフを作成する.
    ...
    if uc_data.config.get("formulation_type") != "delta-kW-bid":
        ...
        _tert_down_header = "Tertiary (Down)"
    elif uc_data.config["optimization_timing"] == "day-ahead":
        _tert_down_header = "Tertiary Reserve (Down) [ΔkW]"
    elif uc_data.config["optimization_timing"] == "intra-day":
        _tert_down_header = "Tertiary Energy (Down) [kWh]"
    else:
        _tert_down_header = "Tertiary (Down)"
    _header_col = [_tert_down_header] + list(timeline.keys().strftime(time_format))
    ...
    for g_type in uc_dicts.generation_type:
        ...
        for time in timeline:
            if uc_data.config["optimization_timing"] == "day-ahead":
                ...
                _value = sum(
                    uc_dicts.p_delta_kW_tert_down[time, name, g_type, area].X
    ...
    for time in timeline:
        if uc_data.config["optimization_timing"] == "day-ahead":
            # 直接アクセス + Pythonのsum()を使用（高速化）
            _value = sum(
                uc_dicts.p_ess_delta_kW_tert_down[time, name, area].X for name, area in ess_list
            )
        elif uc_data.config["optimization_timing"] == "intra-day":
            # 直接アクセス + Pythonのsum()を使用（高速化）
            _value = sum(uc_dicts.p_ess_id_down[time, name, area].X for name, area in ess_list)
        else:
            _value = 0
        _value_col.append(_value)
    _append_col(ws, _value_col)
