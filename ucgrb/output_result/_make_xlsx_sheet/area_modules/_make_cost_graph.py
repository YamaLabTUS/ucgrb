...
def make_cost_graph(ws, period_name, timeline, time_format, area, m, uc_data, uc_dicts):
    """
    各地域のコストグラフを作成する.
    ...
    # 高速化のため、該当するESSのリストを事前に取得（一度だけ生成）
    filtered_ess_list = [(name, area_item) for name, area_item in uc_dicts.ess.select("*", area)]
    ...
    for time in timeline:
        _value = sum(
            uc_dicts.e_ess_short[time, name, area_item].X
            * uc_dicts.ess_para["C_ess_short"][name, area_item]
            * tsg_ratio
            for name, area_item in filtered_ess_list
        )
        _value_col.append(_value)
        _sum += _value
    _value_col.append(_sum)
    ...
    for time in timeline:
        _value = sum(
            uc_dicts.e_ess_surplus[time, name, area_item].X
            * uc_dicts.ess_para["C_ess_surplus"][name, area_item]
            * tsg_ratio
            for name, area_item in filtered_ess_list
        )
        _value_col.append(_value)
    ...
    if uc_data.config.get("formulation_type") != "delta-kW-bid":
        ...
        if hasattr(uc_dicts, "p_ess_delta_kW_tert_up") and hasattr(
            ...
            for time in timeline:
                _value = sum(
                    uc_dicts.p_ess_delta_kW_tert_up[time, name, area_item].X
                    * uc_dicts.ess_para["C_ess_delta_kW_tert_UP"][name, area_item]
                    * tsg_ratio
                    for name, area_item in filtered_ess_list
                )
                _value_col.append(_value)
            ...
            for time in timeline:
                _value = sum(
                    uc_dicts.p_ess_delta_kW_tert_down[time, name, area_item].X
                    * uc_dicts.ess_para["C_ess_delta_kW_tert_DOWN"][name, area_item]
                    * tsg_ratio
                    for name, area_item in filtered_ess_list
                )
                _value_col.append(_value)
        ...
        if (
            ...
            for time in timeline:
                _value = sum(
                    uc_dicts.p_ess_id_up[time, name, area_item].X
                    * uc_dicts.ess_para["C_ess_kWh_UP"].get((name, area_item), 0)
                    * tsg_ratio
                    for name, area_item in filtered_ess_list
                )
                _value_col.append(_value)
            ...
            for time in timeline:
                _value = sum(
                    uc_dicts.p_ess_id_down[time, name, area_item].X
                    * uc_dicts.ess_para["C_ess_kWh_DOWN"].get((name, area_item), 0)
                    * tsg_ratio
                    for name, area_item in filtered_ess_list
                )
                _value_col.append(_value)
