...
# -*- coding: utf-8 -*-
"""
GF & LFC (Up) グラフ生成モジュール.

各地域のGF&LFC上げ調整力を示すグラフを作成する。
"""
from .._append_col import _append_col
from .._make_constraint_chart import _make_constraint_chart


def make_gf_lfc_up_graph(ws, period_name, timeline, time_format, area, m, uc_data, uc_dicts):
    """
    各地域のGF&LFC上げ調整力グラフを作成する.
    ...
    for g_type in uc_dicts.generation_type:
        ...
        for time in timeline:
            ...
            _value = sum(
                uc_dicts.p_gf_lfc_up[time, name, g_type, area].X
    ...
    for time in timeline:
        # 直接アクセス + Pythonのsum()を使用（高速化）
        _value = sum(uc_dicts.p_ess_gf_lfc_up[time, name, area].X for name, area in ess_list)
        _value_col.append(_value)
    _append_col(ws, _value_col)
