...
各地域の需給バランスを示すグラフを作成する。
"""
from .._append_col import _append_col
from .._make_constraint_chart import _make_constraint_chart


def make_power_balance_graph(ws, period_name, timeline, time_format, area, m, uc_data, uc_dicts):
    """
    各地域の需給バランスグラフを作成する.
    ...
    for time in timeline:
        _value = sum(
            uc_dicts.p_tie_f[time, name, f, t].X for name, f, t in tie_list_inflow if t == area
        ) + sum(
            uc_dicts.p_tie_c[time, name, f, t].X for name, f, t in tie_list_inflow if f == area
        )
        _value_col.append(_value)
    _append_col(ws, _value_col)
    ...
    for time in timeline:
        _value = -sum(
            uc_dicts.p_tie_c[time, name, f, t].X for name, f, t in tie_list_inflow if t == area
        ) - sum(
            uc_dicts.p_tie_f[time, name, f, t].X for name, f, t in tie_list_inflow if f == area
        )
        _value_col.append(_value)
    _append_col(ws, _value_col)
    ...
    # グラフに含める列数を計算
    _len_bargraph = len(uc_dicts.generation_type) + 11
    ...
    if uc_data.config["optimization_timing"] == "intra-day":
        ...
        if (
            ...
            for g_type in uc_dicts.generation_type:
                ...
                # 該当する発電機のリストを事前に取得（高速化のため）
                generation_list_da = list(uc_dicts.generation.select("*", g_type, area))
                for time in timeline:
                ...
                # 該当する発電機のリストを事前に取得（高速化のため）
                generation_list_id = list(uc_dicts.generation.select("*", g_type, area))
                for time in timeline:
                    _value = sum(
        ...
        if (
            ...
            # ESSのリストを事前に取得（高速化のため）
            ess_list_da = list(uc_dicts.ess.select("*", area))
            for time in timeline:
