...
全地域のPV（太陽光発電）出力を示すグラフを作成する。
"""
from .._append_col import _append_col
from .._make_constraint_chart import _make_constraint_chart


def make_pv_graph(ws, period_name, timeline, time_format, m, uc_data, uc_dicts):
    """
    全地域のPV出力グラフを作成する.
    ...
    for time in timeline:
        # 直接アクセス + Pythonのsum()を使用（高速化）
        _value = sum(uc_dicts.p_pv_tert_down[time, area].X for area in area_list)
        _value_col.append(_value)
    _append_col(ws, _value_col)
    ...
    for time in timeline:
        # 直接アクセス + Pythonのsum()を使用（高速化）
        _value = sum(uc_dicts.p_pv_gf_lfc_down[time, area].X for area in area_list)
        _value_col.append(_value)
    _append_col(ws, _value_col)
    ...
    for time in timeline:
        # 直接アクセス + Pythonのsum()を使用（高速化）
        _value = sum(uc_dicts.p_pv_gf_lfc_up[time, area].X for area in area_list)
        _value_col.append(_value)
    _append_col(ws, _value_col)
    ...
    for time in timeline:
        # 直接アクセス + Pythonのsum()を使用（高速化）
        _value = sum(uc_dicts.p_pv_tert_up[time, area].X for area in area_list)
        _value_col.append(_value)
    _append_col(ws, _value_col)
